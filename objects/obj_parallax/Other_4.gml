...
for (var i = 0; i < array_length(l); i++)
{
	...
	if !is_undefined(_cur_asset_parallax)
	{
		...
		for (var j = 0; j < array_length(_sprites); ++j)
		{
			...
			layer_sprite_x(_cur_sprite, (_prev_x * ( 1 - _cur_asset_parallax[0])) + ((SCREEN_WIDTH / 2) * _cur_asset_parallax[0]))
			layer_sprite_y(_cur_sprite, (_prev_y * ( 1 - _cur_asset_parallax[1])) + (((SCREEN_HEIGHT / 2) + obj_camera.constant_y_offset) * _cur_asset_parallax[1]))
		}
	}
}

...
repeat opponent.maxhp
{
	...
	if curhp <= opponent.hp
	{
		...
		opponent.hphudpos.x = xx
		opponent.hphudpos.y = yy
	}
	xx += dist
...
player.hpix = wrap(sprite_get_number(player.hpspr), player.hpix + img_spd)
opponent.hpix = wrap(sprite_get_number(opponent.hpspr), opponent.hpix + img_spd)
